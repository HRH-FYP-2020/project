...
class Table extends React.Component{
    constructor(props){
        ...
        recommendation(){
            const url = ''+window.location.href
             email = ''+url.split('http://localhost:3006/memberShip/viewData/')
            const user ={
              email:email[1]
            }
  ...
  Back(){
    window.location.assign('/memberShip/request')
  }


    acceptRequest=(e)=>{
        const url = ''+window.location.href
            const email = url.split('http://localhost:3006/memberShip/viewData/')
        var user={
          email: email,   
        }
        axiosInstance.post('/membership/request/accept',{user})
        .then(res =>{
